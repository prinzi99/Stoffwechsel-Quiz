...
const STORAGE_KEY = "exitIntentShown_dfgk34k-3";

const ExitIntentPopup = () => {
  const [open, setOpen] = useState(false);

  useEffect(() => {
    // Nur Desktop (kein Touch-Hauptgerät)
    if (window.matchMedia("(pointer: coarse)").matches) return;
    if (sessionStorage.getItem(STORAGE_KEY)) return;

    const handleMouseLeave = (e: MouseEvent) => {
      // Nur wenn Maus oben aus dem Viewport geht
    ...
    // Kleine Verzögerung, damit es nicht direkt beim Laden triggert
